...
|------------|----------|----------|----------|------|
| **I2C1 总线（慢速组 100kHz — 3 设备）** |||||
| PB6 | I2C1_SCL | AHT20 / BH1750 / SSD1306 | SCL | 温湿度+光照+显示 |
| PB7 | I2C1_SDA | 同上 | SDA | 地址: 0x38/0x23/0x3C |
| **I2C2 总线（快速组 400kHz — 3 设备）** |||||
| PB10 | I2C2_SCL | BMP280 / MPU6050 / VL53L0X | SCL | 气压+IMU+ToF |
...
| **Timer 3 Encoder Mode（修正）** ||||
| PA6 | TIM3_CH1 | 旋转编码器 | A (DT) |
| PA7 | TIM3_CH2 | 旋转编码器 | B (CLK) |
...
| **GPIO 中断输入** ||||
| PB1 | GPIO_EXTI1 | HC-SR501 PIR | OUT | 上升沿，人体检测 |
...
```
        I2C1 (100kHz)                              I2C2 (400kHz)
        PB6(SCL) ───┬────────┬────────┐             PB10(SCL) ───┬────────┬────────┐
                    │        │        │                           │        │        │
        PB7(SDA) ───┼────────┼────────┼── 4.7kΩ→3.3V  PB11(SDA) ─┼────────┼────────┼── 4.7kΩ→3.3V
                    │        │        │                           │        │        │
                 AHT20   BH1750   SSD1306                      BMP280   MPU6050  VL53L0X
                 (0x38)  (0x23)   (0x3C)                       (0x76)   (0x68)   (0x29)
```
...
| 阶段 | 内容 | 预计 |
|------|------|------|
| **Phase 0** | 面包板接线，I2C 总线扫描，确认所有设备地址 | 先做 |
| **Phase 1** | STM32 传感器驱动层（I2C 5设备 + ADC + 1-Wire） | 代码 |
